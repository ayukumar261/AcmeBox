...
1. Call the `paymentMethods_list` tool to fetch the customer's current payment
   methods. Do this every time, even if you have already seen payment methods
   elsewhere (for example embedded in a `customers_getById` response) —
   `paymentMethods_list` is the authoritative source and the required check
   before any change.
2. Confirm with the customer which of the listed cards they mean (match by brand
   and last 4 digits).
